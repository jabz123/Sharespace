...
<head>
    ...
    *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .form-group label { display: block; font-size: 11.5px; font-weight: 600; color: var(--ink-muted); letter-spacing: 0.05em; text-transform: uppercase; margin-bottom: 6px; }
    .form-row-between { display: flex; align-items: center; justify-content: space-between; margin-bottom: 6px; }
    .form-row-between label { font-size: 11.5px; font-weight: 600; color: var(--ink-muted); letter-spacing: 0.05em; text-transform: uppercase; margin-bottom: 0; }
    .forgot-link { font-size: 12px; color: var(--amber-d); font-weight: 500; text-decoration: none; }
    .forgot-link:hover { text-decoration: underline; }

    .input-shell { position: relative; display: flex; align-items: center; }
    .input-shell .pre-icon { position: absolute; left: 13px; pointer-events: none; color: var(--silver); display: flex; align-items: center; }
    ...
    .input-shell input::placeholder { color: #b0bdd0 !important; font-weight: 300; }
    .input-shell input:hover { border-color: #c8d4e3 !important; background: var(--white) !important; }
    .input-shell input:focus { border-color: var(--amber) !important; background: var(--white) !important; box-shadow: 0 0 0 3.5px var(--amber-glow) !important; }

    .eye-btn { position: absolute; right: 12px; background: none; border: none; cursor: pointer; color: var(--silver); display: flex; align-items: center; padding: 4px; border-radius: 4px; transition: color 0.15s; }
    ...
    .btn-hero::before { content: ''; position: absolute; inset: 0; background: linear-gradient(135deg, rgba(245,158,11,0.18) 0%, transparent 60%); opacity: 0; transition: opacity 0.2s; }
    .btn-hero:hover { transform: translateY(-1px); box-shadow: 0 8px 24px rgba(10,15,30,0.18); opacity: 1 !important; }
    ...
    .or-divider::before, .or-divider::after { content: ''; flex: 1; height: 1px; background: var(--fog); }
    ...
    .status-pill { display: flex; align-items: center; gap: 6px; background: var(--snow); border: 1px solid var(--fog); border-radius: 20px; padding: 5px 11px 5px 8px; font-size: 11.5px; color: var(--slate); font-weight: 500; }
    ...
    .bg-glow-amber { position: absolute; bottom: -80px; left: -160px; width: 480px; height: 480px; background: radial-gradient(circle, rgba(245,158,11,0.07) 0%, transparent 65%); pointer-events: none; }

    .r-topbar { position: relative; z-index: 10; display: flex; align-items: center; justify-content: space-between; padding: 20px 40px; border-bottom: 1px solid var(--card-border); }
    ...
    .a-card { background: rgba(255,255,255,0.035); border: 1px solid var(--card-border); border-radius: 13px; padding: 14px 16px; display: flex; gap: 14px; align-items: flex-start; cursor: pointer; position: relative; overflow: hidden; transition: background 0.18s, border-color 0.18s, transform 0.18s; animation: fadeUp 0.5s cubic-bezier(0.22,1,0.36,1) both; }
    .a-card::before { content: ''; position: absolute; left: 0; top: 0; bottom: 0; width: 3px; background: var(--amber); opacity: 0; transition: opacity 0.18s; }
    .a-card:hover { background: rgba(255,255,255,0.055); border-color: rgba(255,255,255,0.1); transform: translateX(3px); }
    .a-card:hover::before { opacity: 1; }
    .a-card:nth-child(2) { animation-delay: 0.07s; } .a-card:nth-child(3) { animation-delay: 0.14s; } .a-card:nth-child(4) { animation-delay: 0.21s; } .a-card:nth-child(5) { animation-delay: 0.28s; }
    ...
    @media (max-width: 860px) { .panel-right { display: none; } .panel-left { width: 100%; } }
    </style>
</head>
<body>
...
<div class="panel-left">
    ...
    <div class="left-footer">
        <p class="signup-cta">Don't have an accountssss? <a href="/register.php">Sign up free</a></p>
