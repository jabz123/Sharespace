...
if ($auth->currentUser()) { header('Location: ' . BASE_PATH . '/dashboard.php'); exit; }
$error = null;
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    $result = $auth->register($_POST['name'] ?? '', $_POST['email'] ?? '', $_POST['password'] ?? '', $_POST['confirm_password'] ?? '');
    if (isset($result['ok'])) { redirect(BASE_PATH . '/login.php', null, 'Account created! You can now sign in.'); }
    $error = $result['error'];
}
?>
<!DOCTYPE html>
            ...
            <form method="POST" autocomplete="on">
                ...
                <div class="form-group">
                    ...
                    <div class="input-icon" style="position:relative">
                        ...
                        <button type="button" data-toggle-password="password" style="position:absolute;right:12px;top:50%;transform:translateY(-50%);background:none;border:none;cursor:pointer;font-size:16px">👁</button>
                    </div>
                </div>
                <div class="form-group">
