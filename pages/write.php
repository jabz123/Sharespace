...
//handle writing new article and editing existing article. 
//form will post back to itself here
//n8n will call this page to submit new article or update existing article, with ai verification result in session.
//a lot of the logic is for ai verification workflow
...
//builds fingerprint for the article based on its content and metadata to match with AI verification result
...
//resolves stored verification for article if fingerprint matches, 
//used to restore verification result after article is saved and page is reloaded
...
//persists AI verification result to the article record, 
//called after article is saved to store the verification result for future reference
...
//finds recently submitted pending article by the same author with matching content 
//to handle edge case where article is submitted for review but page fails to redirect, 
//so that we can still show the article submitted for review page if user tries to submit again within short time window
...
//only can select an actual image file
...
// if editing existing article and article has pending review notice, show the notice and clear it from record so it wont show again
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'draft') {
        ...
        //storedVerification is to recover saved verification from article record
        ...
        $hasPassingVerification = $isVerificationCurrent
            ...
            && in_array($verifiedDecision, ['auto_publish', 'needs_review'], true);
...
//reconstructs and reuses article fact check state, 
//to prepare form and ui values to display on page later.
...
$verification = $_SESSION['article_ai_verification'] ?? null;
//build fingerprint and compare with saved verification to see if can reuse
$verificationFingerprint = buildArticleVerificationFingerprint($currentVerificationInput, (int) $user->id);
//if editing existing article, also check if stored verification on article record can be reused to restore state after page reload
...
//determine whether have current verification that can be reused to restore state, and extract values to prepare for form display
...
//checks if stored verification belongs to same draft
...
<script>
    ...
    function jumpToClaimHighlight(claimKey) {
        ...
        if (contentFormGroup) {
            contentFormGroup.scrollIntoView({
                behavior: 'smooth',
                block: 'center'
            });
        }
