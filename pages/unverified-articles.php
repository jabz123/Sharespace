...
//for cat expert to see all articles 60-80%
//and to decide whether to verify(post) or unverify(send back to user draft)
...
//onlt cat expert can see this
if ($user->role !== 'category_admin') {
    pageRedirect('/dashboard.php');
}
//handle post request to verify or unverify article
//will call reviewPendingArticle in AdminController to update review status 
...
//get article id from query param if expert click into article to review details
$detailId = (int) ($_GET['id'] ?? 0);
//fetch categories assigned to this expert to determine which articles to show, if no assigned category, show empty state
...
//fetch article details and review progress if expert click into article to review
$detailRow = $detailId ? $adminCtrl->getUnverifiedArticleForExpert((int) $user->id, $detailId) : null;

//if detailId provided but no article found, redirect back to list with error
$detailArticle = $detailRow ? new Article($detailRow) : null;
//extra check to prevent expert access article that is not assigned to them or already reviewed by them
...
<div class="dashboard-layout user-dashboard-shell">
    ...
    <main>
        ...
        <?php if ($detailArticle): ?> <!-- display specific article if id provided -->
        ...
        <?php else: ?> <!-- if no id provided show all unverified articles -->
